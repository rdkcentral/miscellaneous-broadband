...
extern "C" bool ovs_agent_api_init(OVS_COMPONENT_ID cid)
{
    if (!g_ovsMock)
    {
        return false;
    }
    return g_ovsMock->ovs_agent_api_init(cid);
}

extern "C" bool ovs_agent_api_deinit(void)
{
    if (!g_ovsMock)
    {
        return false;
    }
    return g_ovsMock->ovs_agent_api_deinit();
}
