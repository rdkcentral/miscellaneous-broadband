...
#define MQTT_DOCSIS_STATS_TOPIC  "local/device_optimizer_stats"
// #define MQTT_DOCSIS_STATS_TOPIC "local/docsis_stats"
...
bool docsis_stats_publish_data(void *data, long data_len) {
    ...
    if (mosquitto_publish(mosq, &mid, MQTT_DOCSIS_STATS_TOPIC, data_len, data, 0, false) != MOSQ_ERR_SUCCESS)
    {
        log_message("[MQTT] %s: MQTT stats publish failed to local broker\n", __FUNCTION__);
    }
    log_message("[MQTT] %s: Published %ld bytes to topic: %s\n", __FUNCTION__, data_len, MQTT_DOCSIS_STATS_TOPIC);
